...
Route::prefix('install')->name('install.')->middleware(['web', RedirectIfInstalled::class])->group(function () {
...
Route::middleware(['web', CheckInstallation::class])->group(function () {
    ...
    Route::get('/{slug}', [PageController::class, 'show'])->name('page.show')->where('slug', '^(?!install|login|register|password|email|auth|logout|home|dashboard|exams|cart|checkout|lang|admin).*$');
